...
int lcd_draw_scale_jpeg(const char *pathname, int x, int y, int dst_w, int dst_h)
{
    ...
    for (int j = 0; j < dst_h; j++)
    {
        ...
        for (int i = 0; i < dst_w; i++)
        {
            ...
            // 获取四个相邻像素(RGB格式)
            unsigned char *p11 = (unsigned char *)&jpeg_buffer[(y1 * src_w + x1) * 3];
            unsigned char *p12 = (unsigned char *)&jpeg_buffer[(y1 * src_w + x2) * 3];
            unsigned char *p21 = (unsigned char *)&jpeg_buffer[(y2 * src_w + x1) * 3];
            unsigned char *p22 = (unsigned char *)&jpeg_buffer[(y2 * src_w + x2) * 3];
